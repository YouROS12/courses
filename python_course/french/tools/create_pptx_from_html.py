#!/usr/bin/env python3
"""
Create PowerPoint presentations from HTML slide files
Parses HTML content and creates native PowerPoint slides with text
"""
    ...
    for slide in slides:
        ...
        if h1:
            ...
            if h2:
                ...
                # Remove emojis from title for cleaner look
            ...
            for div in slide.find_all('div', recursive=True):
                # Skip if it's a nested slide
                if 'slide' in div.get('class', []):
                    continue

                # Get h3 headers and their content
            ...
            # Also check for direct list items or key points
        ...
        # Check if it's a practice slide
...
def add_title_slide(prs, slide_info):
    ...
    # Add background gradient (simulated with a blue rectangle)
    left = top = Inches(0)
    ...
    if slide_info['content']:
        y_pos = 3.8
        for content in slide_info['content'][:2]:  # Max 2 lines
            subtitle_box = slide.shapes.add_textbox(
                Inches(1), Inches(y_pos), Inches(8), Inches(0.6)
            )
...
def add_content_slide(prs, slide_info):
    ...
    bg_shape.fill.solid()
    bg_shape.fill.fore_color.rgb = RGBColor(255, 255, 255)  # White
    bg_shape.line.fill.background()

    # Add blue header bar
    header_shape = slide.shapes.add_shape(
        1,  # Rectangle
        Inches(0), Inches(0), prs.slide_width, Inches(0.15)
    )
    ...
    underline_shape.fill.fore_color.rgb = RGBColor(0, 102, 204)  # Blue
    underline_shape.line.fill.background()

    # Add content
    if slide_info['content']:
        content_box = slide.shapes.add_textbox(
            Inches(0.7), Inches(1.5), Inches(8.6), Inches(4)
        )
        content_frame = content_box.text_frame
        content_frame.word_wrap = True

        # Add each content item
        for i, content in enumerate(slide_info['content'][:10]):  # Max 10 items
            if i == 0:
            ...
            # If it starts with bullet, adjust level
...
def add_practice_slide(prs, slide_info):
    ...
    bg_shape.fill.solid()
    bg_shape.fill.fore_color.rgb = RGBColor(0, 102, 204)  # Blue
    bg_shape.line.fill.background()
...
def main():
    """Create PowerPoint presentations for chapters 6-10"""
    ...
    print("Creating PowerPoint presentations for chapters 6-10\n")
    print("=" * 60)
    ...
    print("=" * 60)
    print("All PowerPoint presentations created successfully!")
